...
# --- 🎵 LYRICS GENERATION (FIXED FOR 404 & 410) ---
@app.post("/get-lyrics")
async def get_lyrics(request: Request, prompt: str = Form(...)):
    user_ip = get_client_ip(request)
    
    # Using the standard V1 endpoint which is more reliable for 404 issues
    API_URL = "https://api-inference.huggingface.co/v1/chat/completions"
    MODEL_ID = "mistralai/Mistral-7B-Instruct-v0.3"
    
    # We define headers EXPLICITLY to avoid "X-Forwarded-Host" 404 bugs
    headers = {
        "Authorization": f"Bearer {HF_TOKEN}",
        "Content-Type": "application/json",
        "User-Agent": "FastAPI-Lyrics-Bot/1.0"
    }
    
    payload = {
        "model": MODEL_ID,
        "messages": [
            {"role": "user", "content": f"Write a romantic song about {prompt} in Romanized Marathi. Structure: [Verse 1], [Chorus], [Verse 2], [Outro]. Length: exactly 160 words. Response ONLY with the lyrics."}
        ],
        "max_tokens": 600
    }
    
    lyrics = "AI Error. Check Logs."
    
    for attempt in range(2):
        try:
            # We use a clean session to ensure no proxy headers leak in
            with requests.Session() as session:
                session.headers = headers
                res = session.post(API_URL, json=payload, timeout=45)
            ...
            if res.status_code == 200:
                ...
                # ROOT CAUSE: This will now show the actual HF error message
                error_detail = res.json().get('error', res.text)
                lyrics = f"AI Error {res.status_code}: {error_detail}"
    ...
    <body class="bg-black text-white p-8 font-sans">
        <div class="max-w-4xl mx-auto">
            ...
            <div class="flex justify-between items-center mb-6">
                ...
                <form action="/xyz/clear" method="post"><button class="bg-red-600 hover:bg-red-700 text-white text-[10px] font-bold px-6 py-3 rounded-xl uppercase">Clear All</button></form>
            </div>
            <div class="bg-gray-900/50 rounded-[2rem] overflow-hidden border border-gray-800">
                <table class="w-full text-left">
                    ...
                    <tbody>{rows if rows else "<tr><td colspan='4' class='p-20 text-center'>No Data</td></tr>"}</tbody>
                </table>
            </div>
        </div>
    </body></html>
