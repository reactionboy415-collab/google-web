...
# --- 🎵 LYRICS GENERATION (FIXED FOR 410 ERROR) ---
@app.post("/get-lyrics")
async def get_lyrics(request: Request, prompt: str = Form(...)):
    user_ip = get_client_ip(request)
    
    # NEW ROUTER URL (Mandatory for 2026)
    ...
    # New V1 Chat Completion format
    payload = {
        "model": MODEL_ID,
        "messages": [
            {
                "role": "user", 
                "content": f"Write a romantic song about {prompt} in Romanized Marathi. Structure: [Verse 1], [Chorus], [Verse 2], [Outro]. Length: exactly 160 words. Response ONLY with the lyrics. No intros."
            }
        ],
        "max_tokens": 600,
        "temperature": 0.7
    ...
    for attempt in range(3):
        try:
            ...
            if res.status_code == 200:
                ...
                break
            elif res.status_code in [503, 429]: # Loading or Rate Limit
                await asyncio.sleep(10)
                continue
            else:
                lyrics = f"AI Error {res.status_code}. Try again."
        except:
