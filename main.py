...
HF_TOKEN = os.getenv("HF")  # HuggingFace Token
...
@app.get("/")
async def home():
    return {"status": "Backend Live (HF Fixed)"}

# =======================
# 🎵 LYRICS GENERATION (HF FIXED)
# =======================
@app.post("/get-lyrics")
async def get_lyrics(request: Request, prompt: str = Form(...)):
    user_ip = get_client_ip(request)

    # ✅ STABLE HF INFERENCE ENDPOINT
        ...
        try:
            ...
            if res.status_code == 200:
                ...
                if isinstance(data, list) and "generated_text" in data[0]:
                    lyrics = data[0]["generated_text"].strip()
                    break
                else:
                    lyrics = "AI Error: Invalid response format"
                    break
    ...
    history.append(
        {
            "ip": user_ip,
            "topic": prompt[:30],
            "time": (datetime.now() + timedelta(hours=5, minutes=30)).strftime(
                "%H:%M:%S"
            ),
            "status": "Ready",
        }
