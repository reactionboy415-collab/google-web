...
HF_TOKEN = os.getenv("HF") # Your Hugging Face Token
...
@app.get("/")
async def home(): return {"status": "Backend Live (HF Edition)"}

# --- 🎵 LYRICS GENERATION (HUGGING FACE WITH RETRY LOGIC) ---
@app.post("/get-lyrics")
async def get_lyrics(request: Request, prompt: str = Form(...)):
    ...
    payload = {
        "inputs": f"<s>[INST] Write a romantic song about {prompt} in Romanized Marathi. Structure: [Verse 1], [Chorus], [Verse 2], [Outro]. Length: exactly 160 words. Response ONLY with the lyrics. No intros. [/INST]</s>",
        "parameters": {"max_new_tokens": 500, "return_full_text": False},
        "options": {"wait_for_model": True} # <--- Crucial fix for "Busy" error
    }
    
    lyrics = "HF Engine Busy. Try again."
    
    # Retry loop to handle cold-starts or busy periods
    for attempt in range(3):
        try:
            res = requests.post(API_URL, headers=headers, json=payload, timeout=60) # Increased timeout
            if res.status_code == 200:
                data = res.json()
                if isinstance(data, list) and len(data) > 0:
                    lyrics = data[0].get('generated_text', lyrics).strip()
                    break
            elif res.status_code == 503: # Model loading
                await asyncio.sleep(10)
                continue
            else:
                lyrics = f"HF Error: {res.status_code}"
        except:
    ...
    <body class="bg-black text-white p-8 font-sans">
        <div class="max-w-4xl mx-auto">
            <div class="flex justify-between items-center mb-10 bg-blue-900/10 p-8 rounded-[2rem] border border-blue-500/20">
                <div><p class="text-[10px] text-blue-500 font-bold uppercase">Server IP (Render)</p><p class="text-3xl font-mono">{srv_ip}</p></div>
                <div class="text-right"><p class="text-[10px] text-gray-500 font-bold uppercase">Database Size</p><p class="text-3xl font-mono">{len(data)}</p></div>
            </div>
            <div class="flex justify-between items-center mb-6">
                <h1 class="text-2xl font-black italic">ADMIN LOGS</h1>
                <form action="/xyz/clear" method="post"><button class="bg-red-600 hover:bg-red-700 text-white text-[10px] font-bold px-6 py-3 rounded-xl uppercase">Clear All Activity</button></form>
            </div>
            <div class="bg-gray-900/50 rounded-[2rem] overflow-hidden border border-gray-800">
                <table class="w-full text-left font-sm">
                    ...
                    <tbody>{rows if rows else "<tr><td colspan='4' class='p-20 text-center text-gray-700 font-bold uppercase'>No Data</td></tr>"}</tbody>
                </table>
            </div>
        </div>
    </body></html>
