...
@app.get("/")
async def home(): return {"status": "Backend Live (HF Router Edition)"}

# --- 🎵 LYRICS GENERATION (FIXED FOR ROUTER) ---
@app.post("/get-lyrics")
async def get_lyrics(request: Request, prompt: str = Form(...)):
    user_ip = get_client_ip(request)
    
    # NEW MANDATORY ROUTER URL
    ...
    # Updated to V1 Chat Format (Mandatory for Router)
    payload = {
        "model": MODEL_ID,
        "messages": [
            {
                "role": "user", 
                "content": f"Write a romantic song about {prompt} in Romanized Marathi. Structure: [Verse 1], [Chorus], [Verse 2], [Outro]. Length: exactly 160 words. Response ONLY with the lyrics."
            }
        ],
        "max_tokens": 600,
        "stream": False
    ...
    for attempt in range(2):
        try:
            ...
            if res.status_code == 200:
                ...
                # This will show the exact message from HF router to find the root cause
                try:
                    error_json = res.json()
                ...
                lyrics = f"AI Error {res.status_code}: {error_msg}"
                
                if res.status_code == 503: # Model Loading
                    await asyncio.sleep(10)
                    continue
                else:
                    break # Stop if it's a 401, 404, or 400
