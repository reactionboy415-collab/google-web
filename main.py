...
# --- CORS SETUP (InfinityFree ke liye mandatory) ---
app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"],
...
def get_client_ip(request: Request):
    # Render user ki real IP 'X-Forwarded-For' mein bhejta hai
...
@app.get("/")
async def index(): return {"status": "Studio Backend Online"}

# --- STEP 1: FAST LYRICS GENERATOR ---
@app.post("/get-lyrics")
async def get_lyrics(request: Request, prompt: str = Form(...)):
    ...
    system_instruction = (
        f"Write a song about: {prompt}. STRICT RULES: 1. Identify language from prompt. "
        "2. SCRIPT: Use only ENGLISH ALPHABET (Romanized). 3. LENGTH: 150-180 words strictly. "
        "4. Structure: [Verse 1], [Chorus], [Verse 2], [Outro]. No intros/disclaimers."
    )
