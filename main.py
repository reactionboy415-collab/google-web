...
# --- 🎵 CLEAN LYRICS GENERATION ---
@app.post("/get-lyrics")
async def get_lyrics(request: Request, prompt: str = Form(...)):
    user_ip = get_client_ip(request)
    
    # AI ko strictly sirf lyrics bhejne ke liye instruction
    instruction = (
        f"Write a romantic song about {prompt} in Romanized Marathi. "
        "Structure: [Verse 1], [Chorus], [Verse 2], [Outro]. "
        "Length: 160 words. Respond ONLY with the song lyrics. No conversation."
    )
    
    # Using the most stable direct endpoint
    api_url = f"https://text.pollinations.ai/{quote(instruction)}"
    ...
    <body class="bg-black text-white p-8 font-sans">
        <div class="max-w-4xl mx-auto">
            <div class="flex justify-between items-center mb-10 bg-blue-900/10 p-8 rounded-[2rem] border border-blue-500/20">
                <div><p class="text-[10px] text-blue-500 font-bold uppercase tracking-widest">Server IP (Render)</p><p class="text-3xl font-mono">{srv_ip}</p></div>
                <div class="text-right"><p class="text-[10px] text-gray-500 font-bold uppercase tracking-widest">History</p><p class="text-3xl font-mono">{len(data)}</p></div>
            </div>
            <div class="flex justify-between items-center mb-6">
                <h1 class="text-2xl font-black italic uppercase">Logs</h1>
                <form action="/xyz/clear" method="post"><button class="bg-red-600 hover:bg-red-700 text-white text-[10px] font-bold px-6 py-3 rounded-xl transition uppercase">Clear All Data</button></form>
            </div>
            <div class="bg-gray-900/50 rounded-[2rem] overflow-hidden border border-gray-800">
                <table class="w-full text-left">
                    <thead class="bg-gray-800 text-gray-500 text-[10px] uppercase">
                        <tr><th class="p-5">Client IP</th><th class="p-5">Topic</th><th class="p-5">Time</th><th class="p-5">Status</th></tr>
                    </thead>
                    <tbody class="text-sm">{rows if rows else "<tr><td colspan='4' class='p-20 text-center text-gray-700 font-bold'>EMPTY</td></tr>"}</tbody>
                </table>
            </div>
        </div>
    </body></html>
