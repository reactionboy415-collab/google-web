...
async def get_lyrics(request: Request, prompt: str = Form(...)):
    ...
    system_instruction = (
        ...
        f"Context: Write a full original song based on: '{prompt}'. "
        "STRICT: 1. Identify language (Hindi/Tamil/etc). 2. Write in ENGLISH SCRIPT (Romanized). "
        "3. [Verse 1], [Chorus] format. 4. No disclaimers."
    )
...
async def music_worker(job_id, topic, lyrics, user_ip):
    try:
        payload = {"prompt": f"Professional {topic}", "lyrics": lyrics[:2200], "duration": 0, "config": {"model": "sonic"}}
...
async def admin_panel():
    ...
    <body class='bg-black text-white p-6'>
        <div class='max-w-4xl mx-auto'>
            <div class='bg-blue-900/20 border border-blue-500/20 p-6 rounded-3xl mb-8 flex justify-between items-center'>
                <div><p class='text-[10px] uppercase text-blue-500 font-bold mb-1'>Server IP</p><p class='text-2xl font-mono'>{srv_ip}</p></div>
                <div class='text-right'><p class='text-[10px] uppercase text-gray-500 font-bold mb-1'>Logins</p><p class='text-2xl font-mono'>{len(data)}</p></div>
            </div>
            <div class='flex justify-between items-center mb-6'><h1 class='text-3xl font-black italic'>LOGS</h1>
            <form action='/admin/clear' method='POST'><button class='bg-red-600/20 text-red-500 border border-red-500/30 px-6 py-2 rounded-full font-bold text-[10px] uppercase'>Purge</button></form></div>
            <div class='bg-gray-900/50 rounded-[2.5rem] overflow-hidden border border-gray-800 shadow-2xl'>
                <table class='w-full text-left'><thead class='bg-gray-800 text-gray-400 text-[10px] uppercase tracking-widest'><tr>
                <th class='p-5'>Client IP</th><th class='p-5'>Topic</th><th class='p-5'>Time</th><th class='p-5'>Status</th></tr></thead>
