...
ADMIN_PASS = "1234" # APNA PASSWORD YAHAN BADLEIN
...
@app.get("/")
async def home(): return {"status": "Backend Live"}

# --- 🎵 LYRICS GENERATION (STABLE MODEL) ---
@app.post("/get-lyrics")
async def get_lyrics(request: Request, prompt: str = Form(...)):
    user_ip = get_client_ip(request)
    now = datetime.now()
    
    clean_prompt = f"Write a professional song about {prompt} in Romanized Marathi. Structure: [Verse 1], [Chorus], [Verse 2], [Outro]. Length: 160 words."
    # Using Llama-3.3-70B for better stability
    api_url = f"https://text.pollinations.ai/{quote(clean_prompt)}?model=llama&seed={uuid.uuid4().int}"
    
    try:
        res = await asyncio.to_thread(requests.get, api_url, timeout=20)
        lyrics = res.text if res.status_code == 200 else "Server Busy. Try once more."
    except:
...
async def admin_panel(p: str = ""):
    if p != ADMIN_PASS:
        return """<body style="background:#000;color:#fff;display:flex;justify-content:center;align-items:center;height:100vh;font-family:sans-serif;">
                  ...
                  <input type="password" name="p" placeholder="Enter Key" style="background:#111;border:1px solid #333;color:#fff;padding:10px;border-radius:5px;outline:none;">
                  <button type="submit" style="background:#3b82f6;color:#fff;padding:10px 20px;border:none;border-radius:5px;cursor:pointer;">Enter</button>
                  </form></body>"""
    ...
    <body class="bg-black text-white p-8 font-sans">
        <div class="max-w-4xl mx-auto">
            <div class="flex justify-between items-center mb-10 bg-blue-900/10 p-6 rounded-3xl border border-blue-500/20">
                <div><p class="text-xs text-blue-500 font-bold uppercase">Server IP</p><p class="text-2xl font-mono">{srv_ip}</p></div>
                <div class="text-right"><p class="text-xs text-gray-500 font-bold uppercase">Records</p><p class="text-2xl font-mono">{len(data)}</p></div>
            </div>
            <div class="flex justify-between items-center mb-6">
                <h1 class="text-2xl font-black italic">STUDIO LOGS</h1>
                <form action="/xyz/clear" method="post"><button class="bg-red-600 hover:bg-red-700 text-white text-xs font-bold px-6 py-2 rounded-full">CLEAR ALL</button></form>
            </div>
            <div class="bg-gray-900 rounded-3xl overflow-hidden border border-gray-800">
                <table class="w-full text-left">
                    ...
                    <tbody>{rows if rows else "<tr><td colspan='4' class='p-20 text-center text-gray-600'>NO DATA</td></tr>"}</tbody>
                </table>
            </div>
        </div>
    </body></html>
